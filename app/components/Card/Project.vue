<template>
  ...
  >
    <div class="card-top">
      <LazyWindowBar class="bg-transparent text-slate-400"> 
        {{ project.title }}
      </LazyWindowBar>
      <div class="overflow-hidden">
        <LazyNuxtImg
          ...
          :alt="project.title"
          quality="50"
          class="w-full h-50 2xl:h-60 object-cover transition-transform duration-300 ease-in-out group-hover:scale-125 object-top"
        />
      </div>
    </div>
    <div class="px-9 pb-10 h-115 lg:h-100 flex flex-col justify-between">
      <div class="card-content flex flex-col gap-5">
        <div class="card-text flex flex-col gap-5">
          <div
            class="card-title font-semibold font-poppins gap-2.5 text-xl lg:text-[28px] flex"
          >
          ...
          <p class="text-slate-400 text-sm text-[16px] 2xl:text-lg">
            <slot name="resume">{{ project.resume }}</slot>
          </p>
        </div>
      ...
      <div class="card-bottom flex gap-3.5">
        ...
        <LazyButtonCard
          ...
          class="bg-indigo-500 text-indigo-50 hover:bg-indigo-400 dark:bg-emerald-500 dark:text-emerald-50 dark:hover:bg-emerald-300"
        >
          <template #text>Live</template>
          <template #Icon>
